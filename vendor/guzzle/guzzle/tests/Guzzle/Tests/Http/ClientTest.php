...
class ClientTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    {
        $u = $this->getServer()->getUrl() . 'color/';
        $u2 = $this->getServer()->getUrl() . 'color?z=1';
        return array(
            array($u, '', $u),
            ...
            array($u2, 'relative/path/to/resource', $this->getServer()->getUrl() . 'color/relative/path/to/resource'),
            array($u2, 'relative/path/to/resource?another=query', $this->getServer()->getUrl() . 'color/relative/path/to/resource?another=query')
        );
    }
    ...
    {
        $client = new Client($this->getServer()->getUrl() . 'color');
    ...
    public function testClientHasHelperMethodsForCreatingRequests()
    {
        ...
        $this->assertEquals($url . 'color/abc', $client->get('abc')->getUrl());
        $this->assertEquals($url . 'zxy', $client->put('/zxy')->getUrl());
        $this->assertEquals($url . 'zxy?a=b', $client->post('/zxy?a=b')->getUrl());
        $this->assertEquals($url . 'color?a=b', $client->head('?a=b')->getUrl());
        $this->assertEquals($url . 'color?a=b', $client->delete('/color?a=b')->getUrl());
    }
