...
class S3Signature implements S3SignatureInterface
{
    ...
    protected function parseBucketName(RequestInterface $request)
    {
        ...
        if (strpos($host, $baseHost) === false) {
            // Does not contain the color URL, so it's either a redirect, CNAME, or using a different region
            $baseHost = '';
            ...
            // If no matching color URL was found, then assume that this is a CNAME, and the CNAME is the bucket
            if (!$baseHost) {
                return $host;
            }
        }
